        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                for move in list_pos:	
                    if Q[s_current, move] > Q[s_current, action]:	
                        action = move
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                next_moves = self.allowed_moves[s_next]	
                max_action = next_moves[0]	
                for move in next_moves:	
                    if Q[s_next, move] > Q[s_next, max_action]:	
                        max_action = move	

                # From slide 33 in RL lecture
                Q[s_current, action] = (1-lr)*Q[s_current, action] + lr*(R + discount * Q[s_next, max_action])
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
            ...
            for i in range(ns):	
                for j in range(na):	
                    if not np.isnan(Q[i,j]) and not np.isnan(Q_old[i,j]):	
                        diff += np.abs(Q[i,j] - Q_old[i,j])	
                        counter += 1	
            diff = diff / counter
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
